use crate::{
    app::{CLIENT_DATA_FILE_NAME, DisplayInfo, IOA_DATA_FOLDER_NAME, SESSION_DATA_FOLDER_NAME},
    data::ClientData,
...
impl NewClient {
    fn save_file_to_path(&mut self, client_directory_path: &PathBuf) -> Result<()> {
        let mut p = Path::new(client_directory_path).join(self.client.id.to_string());
    ...
    pub fn view(
        ...
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            ui.add_enabled_ui(self.client.id != "None", |ui| {
                if ui
